...
lateralize_tree <- function(tree, hemisphere = "left") {
  
  #Prune tree for lateral regions
  tree_lat <- Clone(tree)
  tree_lat$Do(
    function(node) {
      if (isLeaf(node)) {
        if (hemisphere == "midline") {
          ind_left <- !str_detect(node[["name"]], "^left")
          ind_right <- !str_detect(node[["name"]], "^right")
          node[["keep"]] <- ind_left & ind_right
        } else {
          node[["keep"]] <- str_detect(node[["name"]], str_c("^", hemisphere))          
        }
      } else {
        node[["keep"]] <- Aggregate(node, "keep", any)
      }
    },
    traversal = "post-order"
  )
  Prune(tree_lat, pruneFun = function(node) {node[["keep"]]})
  
  #Aggregate hemisphere atlas labels
  tree_lat$Do(
    function(node) {node$label <- unlist(Aggregate(node, "label", c))},
    traversal = "post-order"
  )
  
  #Remove redundant leaf nodes
  Prune(tree_lat, pruneFun = function(node) {!isLeaf(node)})
  
  #Rename nodes with hemispheric info
  if (hemisphere != "midline") {
    tree_lat$Do(
      function(node) {
        node$name = str_c(hemisphere, node$name, sep = " ")
      }
    )  
  }
  
  return(Clone(tree_lat))
}
